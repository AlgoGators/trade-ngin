...
Result<void> DynamicOptimizer::validate_inputs(
    ...
    if (current_positions.empty() || target_positions.empty() || costs.empty() ||
        ...
        return make_error<void>(ErrorCode::INVALID_ARGUMENT, "Input vectors cannot be empty",
                                "DynamicOptimizer");
    }
    ...
    if (current_positions.size() != target_positions.size() ||
        ...
        std::string size_error =
            "Input vectors must have the same size: " + std::to_string(current_positions.size()) +
            ", " + std::to_string(target_positions.size()) + ", " + std::to_string(costs.size()) +
            ", " + std::to_string(weights_per_contract.size()) + ", " +
            std::to_string(covariance.size());
        return make_error<void>(ErrorCode::INVALID_ARGUMENT, size_error, "DynamicOptimizer");
    }

    for (const auto& row : covariance) {
        if (row.size() != current_positions.size()) {
            return make_error<void>(ErrorCode::INVALID_ARGUMENT, "Covariance matrix must be square",
                                    "DynamicOptimizer");
        }
    }
...
Result<OptimizationResult> DynamicOptimizer::optimize_single_period(
    ...
    if (validation.is_error()) {
        return make_error<OptimizationResult>(validation.error()->code(),
                                              validation.error()->what(), "DynamicOptimizer");
    }
    ...
    try {
        ...
        return make_error<OptimizationResult>(ErrorCode::UNKNOWN_ERROR,
...
Result<OptimizationResult> DynamicOptimizer::apply_buffering(
    ...
    try {
        ...
        if (tracking_error <= buffer_size) {
            DEBUG("Tracking error " + std::to_string(tracking_error) + " is below buffer " +
                  std::to_string(buffer_size) + ". No trading required.");

            // Return current positions with updated metrics
            double current_cost =
                calculate_cost_penalty(current_positions, current_positions, costs);
            double current_error = calculate_tracking_error(target_positions, current_positions,
        ...
        DEBUG("Tracking error " + std::to_string(tracking_error) + " exceeds buffer " +
        ...
        for (size_t i = 0; i < current_positions.size(); ++i) {
            double required_trade =
                (optimized_positions[i] - current_positions[i]) * adjustment_factor;
        ...
        double final_error =
            calculate_tracking_error(target_positions, final_positions, covariance, final_cost);
...
double DynamicOptimizer::calculate_cost_penalty(const std::vector<double>& current_positions,
...
double DynamicOptimizer::calculate_pure_tracking_error(
    const std::vector<double>& target_positions, const std::vector<double>& proposed_positions,
    const std::vector<std::vector<double>>& covariance) const {
    ...
    for (size_t i = 0; i < matrix_size; ++i) {
        ...
        for (size_t j = 0; j < matrix_size; ++j) {
            if (j >= covariance[i].size())
                break;
            tracking_error_squared +=
                tracking_error_weights[i] * covariance[i][j] * tracking_error_weights[j];
        }
    }
...
double DynamicOptimizer::calculate_tracking_error(
    const std::vector<double>& target_positions, const std::vector<double>& proposed_positions,
    const std::vector<std::vector<double>>& covariance, double cost_penalty) const {
    // Calculate pure tracking error (standard deviation component)
    double std_dev =
        calculate_pure_tracking_error(target_positions, proposed_positions, covariance);
...
Result<void> DynamicOptimizer::update_config(const DynamicOptConfig& config) {
    ...
    if (config.tau <= 0.0) {
        return make_error<void>(ErrorCode::INVALID_ARGUMENT,
                                "Risk aversion parameter must be positive", "DynamicOptimizer");
    }
    ...
    if (config.cost_penalty_scalar < 0.0) {
        return make_error<void>(ErrorCode::INVALID_ARGUMENT,
                                "Cost penalty scalar must be non-negative", "DynamicOptimizer");
    }

    if (config.buffer_size_factor < 0.0) {
        return make_error<void>(ErrorCode::INVALID_ARGUMENT,
                                "Buffer size factor must be non-negative", "DynamicOptimizer");
    }
