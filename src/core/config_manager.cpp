        ...
        for (int i = static_cast<int>(ConfigType::STRATEGY); 
            ...
            if (std::filesystem::exists(config_file)) {
                ...
                try {
                    ...
                    return make_error<void>(
                        ErrorCode::INVALID_ARGUMENT,
                        "Invalid JSON in config file: " + config_file.string() + 
                        " - " + e.what(),
                        "ConfigManager"
                    );
                }
                ...
                auto migration_result = ConfigVersionManager::instance().auto_migrate(
                    config_[component], type);
        ...
        for (const auto& entry : std::filesystem::directory_iterator(env_dir)) {
            if (entry.is_regular_file() && entry.path().extension() == ".json") {
                ...
                if (!file.is_open()) {
                    return make_error<void>(
                        ErrorCode::INVALID_ARGUMENT,
                        "Failed to open override file: " + entry.path().string(),
                        "ConfigManager"
                    );
                }
                ...
                try {
                    ...
                    return make_error<void>(
                        ErrorCode::INVALID_ARGUMENT,
                        "Invalid JSON in override file: " + entry.path().string() + 
                        " - " + e.what(),
                        "ConfigManager"
                    );
                }
...
Result<void> ConfigManager::validate_config(
    ...
    if (!errors.empty()) {
        std::stringstream ss;
        ss << "Configuration validation failed for " 
           << get_component_name(component_type) << ":";
...
Result<void> ConfigManager::save_configs() {
    ...
    try {
        ...
        return make_error<void>(
            ErrorCode::UNKNOWN_ERROR,
            std::string("Error saving configs: ") + e.what(),
            "ConfigManager"
        );
    }
}
