...
Result<void> EmailSender::send_email(const std::string& subject, const std::string& body,
    ...
    try {
        ...
        curl_easy_setopt(curl, CURLOPT_VERBOSE,
                         0L);  // Disable verbose output for production
...
std::string EmailSender::generate_trading_report_body(
    ...
    html << "<span class=\"fund-branding\">AlgogAtors</span> - Trend Following "
            "Strategy<br>\n";
    ...
    html << "<hr>\n";
    html << "<p><small>Generated by Trade-ngin Live Pipeline for <span "
            "class=\"fund-branding\">AlgogAtors</span></small></p>\n";
    html << "</body>\n</html>\n";

    return html.str();
}

std::string EmailSender::generate_trading_report_from_db(std::shared_ptr<DatabaseInterface> db,
...
std::string EmailSender::format_positions_table(
    ...
    html << "<table>\n";
    html << "<tr><th>Symbol</th><th>Quantity</th><th>Avg "
            "Price</th><th>Notional</th><th>Unrealized "
            "P&L</th></tr>\n";
    ...
    for (const auto& [symbol, position] : positions) {
        if (position.quantity.as_double() != 0.0) {
            ...
            std::string pnl_class =
                position.unrealized_pnl.as_double() >= 0 ? "positive" : "negative";

            html << "<tr>\n";
            html << "<td>" << symbol << "</td>\n";
            html << "<td>" << std::fixed << std::setprecision(2) << position.quantity.as_double()
                 << "</td>\n";
            html << "<td>$" << std::fixed << std::setprecision(2)
                 << position.average_price.as_double() << "</td>\n";
    ...
    html << "<strong>Total Notional:</strong> $" << std::fixed << std::setprecision(2)
         << total_notional << "\n";
    html << "</div>\n";
...
std::string EmailSender::format_strategy_metrics(
    ...
    for (const auto& [key, value] : strategy_metrics) {
        ...
        // Add appropriate units based on the metric name with proper decimal
        // formatting
        ...
        html << "<strong>" << key << ":</strong> <span" << value_class << ">" << formatted_value
             << "</span><br>\n";
    }
    html << "</div>\n";
