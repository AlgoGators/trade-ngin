...
Result<ConvergenceInfo> GJRGARCH::fit_with_diagnostics(const std::vector<double>& returns) {
    auto result = fit(returns);
    if (result.is_error()) {
        return make_error<ConvergenceInfo>(result.error()->code(), result.error()->what(), "GJRGARCH");
    }
