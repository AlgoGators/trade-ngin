...
TransactionCostAnalyzer::TransactionCostAnalyzer(TCAConfig config) : config_(std::move(config)) {}

Result<TransactionCostMetrics> TransactionCostAnalyzer::analyze_trade(
    const ExecutionReport& execution, const std::vector<Bar>& market_data) const {
    try {
        ...
        auto market_volume_it =
            std::find_if(market_data.begin(), market_data.end(), [&execution](const Bar& bar) {
                return bar.symbol == execution.symbol &&
                       std::abs(std::chrono::duration_cast<std::chrono::minutes>(
                                    bar.timestamp - execution.fill_time)
                                    .count()) < 5;
            });

        if (market_volume_it != market_data.end()) {
            metrics.participation_rate =
                execution.filled_quantity.as_double() / market_volume_it->volume;
        }

        // Calculate price reversion
        if (!market_data.empty() && market_volume_it != market_data.end()) {
            auto post_trade_it =
                std::find_if(market_volume_it, market_data.end(), [&execution](const Bar& bar) {
                    return std::chrono::duration_cast<std::chrono::minutes>(bar.timestamp -
                                                                            execution.fill_time)
                               .count() >= 30;  // Look 30 minutes ahead
                });
        ...
        return make_error<TransactionCostMetrics>(ErrorCode::UNKNOWN_ERROR,
...
Result<TransactionCostMetrics> TransactionCostAnalyzer::analyze_trade_sequence(
    ...
    try {
        ...
        for (const auto& execution : executions) {
            ...
            double trade_value =
                std::abs(execution.filled_quantity.as_double() * execution.fill_price.as_double());
        ...
        if (!executions.empty()) {
            aggregate_metrics.execution_time =
                std::chrono::duration_cast<std::chrono::milliseconds>(executions.back().fill_time -
                                                                      executions.front().fill_time);
        }
        ...
        return make_error<TransactionCostMetrics>(
            ErrorCode::UNKNOWN_ERROR, std::string("Error analyzing trade sequence: ") + e.what(),
            "TransactionCostAnalyzer");
    }
}
...
TransactionCostAnalyzer::analyze_benchmark_performance(
    ...
    try {
        ...
        double execution_vwap = total_quantity > 0.0 ? total_value / total_quantity : 0.0;
        ...
        if (config_.use_vwap) {
            ...
            double market_vwap = market_volume > 0.0 ? market_value / market_volume : 0.0;

            benchmark_metrics["vwap_performance"] = (execution_vwap - market_vwap) / market_vwap;
        }
        ...
        if (config_.use_twap) {
            ...
            if (count > 0) {
                twap /= count;
                benchmark_metrics["twap_performance"] = (execution_vwap - twap) / twap;
            }
        }
...
double TransactionCostAnalyzer::calculate_spread_cost(const ExecutionReport& execution,
                                                      const std::vector<Bar>& market_data) const {
    // Find the closest bar before execution
    auto it =
        std::lower_bound(market_data.begin(), market_data.end(), execution.fill_time,
                         [](const Bar& bar, const Timestamp& ts) { return bar.timestamp < ts; });
...
double TransactionCostAnalyzer::calculate_market_impact(const ExecutionReport& execution,
                                                        const std::vector<Bar>& market_data) const {
    ...
    return price_impact * execution.fill_price.as_double() *
           std::abs(execution.filled_quantity.as_double());
}

double TransactionCostAnalyzer::calculate_timing_cost(const ExecutionReport& execution,
                                                      const std::vector<Bar>& market_data) const {
    // Find optimal execution price in window around trade
    auto window_start = execution.fill_time - config_.pre_trade_window;
    auto window_end = execution.fill_time + config_.post_trade_window;

    double best_price = execution.fill_price.as_double();  // Default to actual price

    for (const auto& bar : market_data) {
        if (bar.timestamp >= window_start && bar.timestamp <= window_end) {
            if (execution.side == Side::BUY) {
...
std::string TransactionCostAnalyzer::generate_report(const TransactionCostMetrics& metrics,
    ...
    report << "Transaction Cost Analysis Report\n"
           ...
           << "  Total Cost: "
           << (metrics.spread_cost + metrics.market_impact + metrics.timing_cost +
               metrics.delay_cost)
           << "\n\n"
           << "Execution Statistics:\n"
           << "  Participation Rate: " << (metrics.participation_rate * 100.0) << "%\n"
           << "  Number of Orders: " << metrics.num_child_orders << "\n"
           << "  Execution Time: " << metrics.execution_time.count() << "ms\n"
           << "  Price Reversion: " << (metrics.price_reversion * 100.0) << "%\n";
