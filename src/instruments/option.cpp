...
Greeks OptionInstrument::calculate_greeks(
    ...
    if (spec_.type == OptionType::CALL) {
        ...
        greeks.theta = (-underlying_price * volatility * norm_pdf(d1)) / 
                      (2 * sqrt_t) - 
                      risk_free_rate * spec_.strike * std::exp(-risk_free_rate * t) * 
                      norm_cdf(d2);
    } else {
    ...
    greeks.rho = (spec_.type == OptionType::CALL ? 1.0 : -1.0) * 
                 spec_.strike * t * std::exp(-risk_free_rate * t) * 
                 norm_cdf(spec_.type == OptionType::CALL ? d2 : -d2) / 100.0;  // Per 1% rate change

    return greeks;
}

std::optional<double> OptionInstrument::calculate_implied_volatility(
    double option_price,
    double underlying_price,
    double risk_free_rate) const {
    ...
    for (int i = 0; i < MAX_ITERATIONS; ++i) {
        double price = calculate_theoretical_price(
            underlying_price, vol_guess, risk_free_rate);
...
double OptionInstrument::calculate_theoretical_price(
    double underlying_price,
    double volatility,
    double risk_free_rate) const {
