...
TrendFollowingFastStrategy::TrendFollowingFastStrategy(std::string id, StrategyConfig config,
    ...
    : BaseStrategy(std::move(id), std::move(config), std::move(db)),
      ...
      registry_(registry) {
    Logger::register_component("Fast Trend Following Strategy");

    // Verify lengths of lookback periods
    if (trend_config_.vol_lookback_short <= 0) {
        trend_config_.vol_lookback_short = 16;
    }
    ...
    metadata_.description = "Multi-timeframe fast trend following using EMA crossovers with shorter windows";
}

Result<void> TrendFollowingFastStrategy::validate_config() const {
...
Result<void> TrendFollowingFastStrategy::initialize() {
    ...
    try {
        ...
        std::cerr << "Error in TrendFollowingFastStrategy::initialize: " << e.what() << std::endl;
        return make_error<void>(
...
Result<void> TrendFollowingFastStrategy::on_data(const std::vector<Bar>& data) {
    // Validate data
    if (data.empty()) {
        return Result<void>();
    }
    ...
    try {
        ...
        ERROR("Error processing data in TrendFollowingFastStrategy: " + std::string(e.what()));
...
double TrendFollowingFastStrategy::compute_long_term_avg(const std::vector<double>& history,
...
std::vector<double> TrendFollowingFastStrategy::blended_ewma_stddev(const std::vector<double>& prices,
...
std::vector<double> TrendFollowingFastStrategy::get_scaled_forecast(
...
std::vector<double> TrendFollowingFastStrategy::get_scaled_combined_forecast(
...
double TrendFollowingFastStrategy::get_abs_value(const std::vector<double>& values) const {
    return std::accumulate(values.begin(), values.end(), 0.0,
                           [](double acc, double val) { return acc + std::abs(val); });
}

std::unordered_map<std::string, double> TrendFollowingFastStrategy::get_weights() const {
...
double TrendFollowingFastStrategy::calculate_position(const std::string& symbol, double forecast,
...
double TrendFollowingFastStrategy::apply_position_buffer(const std::string& symbol, double raw_position,
                                                     double price, double volatility) const {
    if (!trend_config_.use_position_buffering) {
        return raw_position;
    }
...
double TrendFollowingFastStrategy::calculate_vol_regime_multiplier(
...
double TrendFollowingFastStrategy::get_point_value_multiplier(const std::string& symbol) const {
...
std::unordered_map<int, double> TrendFollowingFastStrategy::get_ema_values(const std::string& symbol, const std::vector<int>& windows) const {
...
int TrendFollowingFastStrategy::get_max_required_lookback() const {
    int max_lookback = 0;
    ...
    // Also consider volatility lookback
