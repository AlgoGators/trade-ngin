...
RegimeDetector::RegimeDetector(RegimeDetectorConfig config)
...
Result<void> RegimeDetector::update(const std::vector<Bar>& data) {
    try {
        ...
        for (const auto& bar : data) {
            ...
            if (regime_changed && 
                validate_regime_change(bar.symbol, new_regime, features)) {
...
Result<RegimeDetectionResult> RegimeDetector::get_regime(
    ...
    if (it == current_regimes_.end()) {
        return make_error<RegimeDetectionResult>(
...
Result<RegimeFeatures> RegimeDetector::get_features(
    const std::string& symbol) const {
    ...
    auto it = current_regimes_.find(symbol);
    if (it == current_regimes_.end()) {
        return make_error<RegimeFeatures>(
            ErrorCode::INVALID_ARGUMENT,
...
Result<bool> RegimeDetector::has_regime_changed(
    const std::string& symbol) const {
    ...
    return Result<bool>(
        it->second.regime_duration < config_.min_regime_duration
    );
}

Result<double> RegimeDetector::get_change_probability(
    const std::string& symbol) const {
...
Result<std::vector<RegimeDetectionResult>> RegimeDetector::get_regime_history(
    ...
    if (it == regime_history_.end()) {
        return make_error<std::vector<RegimeDetectionResult>>(
            ErrorCode::INVALID_ARGUMENT,
            "No regime history for symbol: " + symbol,
            "RegimeDetector"
        );
    }
...
double RegimeDetector::calculate_trend_strength(
...
double RegimeDetector::calculate_mean_reversion(
    const std::vector<double>& prices) const {
...
double RegimeDetector::calculate_volatility(
    const std::vector<double>& prices) const {
...
double RegimeDetector::calculate_hurst(
    const std::vector<double>& prices) const {
    
    if (prices.size() < 2) return 0.5;  // Random walk

    std::vector<int> lags = {2, 4, 8, 16, 32};
    std::vector<double> rs_values;
    
    for (int lag : lags) {
        if (static_cast<size_t>(lag) >= prices.size()) break;
        ...
        for (size_t i = 0; i < prices.size() - lag + 1; ++i) {
            ...
            double mean = std::accumulate(
                window.begin(), window.end(), 0.0) / window.size();
            
            double sq_sum = std::inner_product(
                window.begin(), window.end(), window.begin(), 0.0);
    ...
    if (rs_values.empty()) return 0.5;

    return std::accumulate(
        rs_values.begin(), rs_values.end(), 0.0) / rs_values.size();
}

double RegimeDetector::calculate_variance_ratio(
    ...
    {
        double mean = std::accumulate(
            returns.begin(), returns.end(), 0.0) / returns.size();
        double sq_sum = std::inner_product(
            returns.begin(), returns.end(), returns.begin(), 0.0);
    ...
    for (int lag : lags) {
        if (static_cast<size_t>(lag) >= returns.size()) continue;
        ...
        double mean = std::accumulate(
            lagged_returns.begin(), lagged_returns.end(), 0.0) / lagged_returns.size();
        double sq_sum = std::inner_product(
            lagged_returns.begin(), lagged_returns.end(), lagged_returns.begin(), 0.0);
...
DetailedMarketRegime RegimeDetector::detect_regime_change(
...
double RegimeDetector::calculate_change_probability(
    const RegimeFeatures& features,
...
bool RegimeDetector::validate_regime_change(
    const std::string& symbol,
    DetailedMarketRegime new_regime,
    const RegimeFeatures& features) const {
    
    auto it = current_regimes_.find(symbol);
    if (it == current_regimes_.end()) return true;  // First regime
