...
Result<void> TrendFollowingSlowStrategy::initialize() {
    ...
    try {
        ...
        std::cerr << "Error in TrendFollowingSlowStrategy::initialize: " << e.what() << std::endl;
        return make_error<void>(
            ErrorCode::STRATEGY_ERROR,
            std::string("Failed to initialize slow trend following strategy: ") + e.what(),
            "TrendFollowingSlowStrategy");
    }
}
...
Result<void> TrendFollowingSlowStrategy::on_data(const std::vector<Bar>& data) {
    ...
    try {
        ...
        ERROR("Error processing data in TrendFollowingSlowStrategy: " + std::string(e.what()));
...
std::unordered_map<std::string, Position> TrendFollowingSlowStrategy::get_target_positions() const {
...
std::vector<double> TrendFollowingSlowStrategy::ewma_standard_deviation(
...
double TrendFollowingSlowStrategy::get_abs_value(const std::vector<double>& values) const {
    return std::accumulate(values.begin(), values.end(), 0.0,
                           [](double acc, double val) { return acc + std::abs(val); });
}

std::unordered_map<std::string, double> TrendFollowingSlowStrategy::get_weights() const {
...
double TrendFollowingSlowStrategy::get_point_value_multiplier(const std::string& symbol) const {
