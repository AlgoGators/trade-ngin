...
Result<void> ResultsManagerBase::validate_database_connection() const {
    if (!db_) {
        return make_error<void>(ErrorCode::DATABASE_ERROR, "Database pointer is null",
                                component_id_);
    }
...
Result<void> ResultsManagerBase::validate_storage_enabled() const {
    if (!store_enabled_) {
        ...
        return make_error<void>(ErrorCode::INVALID_DATA, "Storage is disabled", component_id_);
    }
    return Result<void>();
}
...
Result<void> ResultsManagerBase::save_positions(const std::vector<Position>& positions,
    ...
    INFO("Storing " + std::to_string(positions.size()) + " positions to " + table_name +
         " for run_id: " + run_id +
         " on date: " + std::to_string(std::chrono::system_clock::to_time_t(date)));
...
Result<void> ResultsManagerBase::save_executions(const std::vector<ExecutionReport>& executions,
    ...
    INFO("Storing " + std::to_string(executions.size()) + " executions to " + table_name);
...
Result<void> ResultsManagerBase::save_signals(
    const std::unordered_map<std::string, double>& signals, const std::string& run_id,
    ...
    INFO("Storing " + std::to_string(signals.size()) + " signals to " + table_name);
