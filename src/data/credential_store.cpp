...
CredentialStore::CredentialStore(const std::string& path, bool use_encryption) 
    ...
    if (env_config) {
        ...
        if (env_path.filename().extension() == ".json" && 
            env_path.string().length() < 512) {
    ...
    if (use_encryption_) {
        ...
        if (key_result.is_error()) {
            ...
            std::cerr << "Warning: Encryption disabled - " << key_result.error()->what() << std::endl;
        } else {
    ...
    if (load_result.is_error()) {
        throw std::runtime_error("Failed to load config: " + std::string(load_result.error()->what()));
    }
}
...
Result<void> CredentialStore::load_config() {
    if (!std::filesystem::exists(config_path_)) {
        return make_error<void>(
            ...
            "Config file not found: " + config_path_,
            "CredentialStore"
        );
    }
    ...
    try {
        ...
        if ((perms & std::filesystem::perms::others_read) != std::filesystem::perms::none) {
            std::cerr << "Warning: Config file has potentially unsafe permissions: " << config_path_ << std::endl;
        }
    ...
    if (!config_file.is_open()) {
        return make_error<void>(
            ...
            "Failed to open config file: " + config_path_,
            "CredentialStore"
        );
    }
...
Result<std::string> CredentialStore::get_encryption_key() const {
    ...
    return make_error<std::string>(
        ...
        "No valid encryption key found. Set TRADING_ENCRYPTION_KEY environment variable or create key file",
        "CredentialStore"
    );
}
...
Result<void> CredentialStore::validate_credential(const std::string& key, const std::string& value) const {
    ...
    if (it != validation_patterns_.end()) {
        ...
        if (!std::regex_match(value, pattern)) {
            return make_error<void>(
                ...
                "Invalid format for credential: " + key,
                "CredentialStore"
            );
        }
    }
...
Result<void> CredentialStore::validate_names(const std::string& section, const std::string& key) const {
...
Result<std::string> CredentialStore::get_credential(const std::string& section, const std::string& key) const {
    auto name_validation = validate_names(section, key);
    if (name_validation.is_error()) {
        return make_error<std::string>(
            name_validation.error()->code(),
...
Result<void> CredentialStore::remove_credential(const std::string& section, const std::string& key) {
    auto name_validation = validate_names(section, key);
    if (name_validation.is_error()) {
        return name_validation;
    }
...
Result<void> CredentialStore::save_config() const {
    try {
        ...
        try {
            ...
            std::cerr << "Warning: Could not set secure file permissions: " << e.what() << std::endl;
        }
