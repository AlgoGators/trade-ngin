...
std::shared_ptr<arrow::Table> DatabaseClient::fetchDataAsArrowTable(
    ...
    std::vector<std::shared_ptr<arrow::TimestampBuilder>> timestamp_builders;
    ...
    for (const auto& field : schema->fields()) {
        if (field->type()->id() == arrow::Type::STRING) {
            ...
            timestamp_builders.push_back(nullptr);
    ...
    try {
        ...
        std::cout << "Converting Arrow Table with " << res.size() << " rows" << std::endl;
        ...
        for (size_t row_idx = 0; row_idx < res.size(); ++row_idx) {
            ...
            for (int i = 0; i < schema->num_fields(); ++i) {
                ...
                try {
                    ...
                    } else if (timestamp_builders[i]) {
                        std::string timestamp_str = row[index].c_str();
                        struct tm tm = {};
                        if (strptime(timestamp_str.c_str(), "%Y-%m-%d %H:%M:%S", &tm) == nullptr) {
                            throw std::runtime_error("Failed to parse timestamp: " + timestamp_str);
                        }
                        time_t timestamp = mktime(&tm);
                        if (timestamp == -1) {
                            throw std::runtime_error("Failed to convert timestamp: " + timestamp_str);
                        }
                        // Convert to microseconds
                        int64_t timestamp_us = static_cast<int64_t>(timestamp) * 1000000;
                        auto status = timestamp_builders[i]->Append(timestamp_us);
                        if (!status.ok()) {
                            throw std::runtime_error("Error appending timestamp value: " + status.ToString());
                        }
                    }
                } catch (const std::exception& e) {
                    throw std::runtime_error(
        ...
        std::cout << "Arrays extracted successfully" << std::endl;
