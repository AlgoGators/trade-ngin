...
public:
    ...
    TrendStrategy(double capital, const StrategyConfig& config)
        : capital_(capital), config_(config) {}
    ...
    DataFrame positions() const {
        return current_positions_;
    }

    void update(const DataFrame& market_data) {
...
private:
    ...
    std::vector<double> calculateVolatility(const std::vector<double>& returns, int window) {
        ...
        for (size_t i = 0; i < returns.size(); ++i) {
            ...
            vol[i] = std::sqrt(sum_sq / count) * std::sqrt(252.0); // Annualized
        }
        return vol;
    }
    ...
    std::vector<double> applyPositionSizing(const std::vector<double>& trend_signal,
        ...
        for (size_t i = 0; i < positions.size(); ++i) {
            ...
            positions[i] = std::max(std::min(positions[i], config_.pos_limit), 
                                  -config_.pos_limit);
        }
        return positions;
    }
