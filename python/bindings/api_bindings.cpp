...
void bind_backtest_api(py::module_& m) {
    py::class_<BacktestRunner>(m, "BacktestRunner")
        ...
        .def("register_strategy", [](BacktestRunner& self, const std::string& strategy_id,
                                     py::object py_class, py::dict py_dict) {
            self.register_strategy(
                strategy_id,
                [py_class, py_dict, strategy_id](
                    const StrategyContext& ctx,
                    const nlohmann::json& strategy_def)  // TODO do we need strategy_def now?
                -> std::shared_ptr<BaseStrategy> {
                    ...
                    // removed and every subsequent get_override call returns empty. The
                    // aliasing constructor shares ownership with py_owner (keeping the
                    // py::object alive) while the pointer seen by callers is the
                    // BaseStrategy*.  The GIL must be held when the py::object is finally
                    // released.
