...
public:
    ...
    AssetType get_type() const override { return AssetType::OPTION; }
     ...
     */
    OptionType get_option_type() const { return spec_.type; }

    /**
     * @brief Get exercise style
     */
    ExerciseStyle get_exercise_style() const { return spec_.style; }

    /**
     * @brief Get strike price
     */
    double get_strike() const { return spec_.strike; }

    /**
     * @brief Get expiration timestamp
     */
    const Timestamp& get_expiry() const { return spec_.expiry; }
     ...
     */
    std::optional<double> calculate_implied_volatility(
        double option_price,
        double underlying_price,
        double risk_free_rate
    ) const;
     ...
     */
    bool is_weekly() const { return spec_.is_weekly; }

    /**
     * @brief Check if option is adjusted
     */
    bool is_adjusted() const { return spec_.is_adjusted; }
...
private:
     ...
     */
    double calculate_theoretical_price(
        double underlying_price,
        double volatility,
        double risk_free_rate
    ) const;
