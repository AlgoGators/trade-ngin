...
public:
    ResultsManagerBase(std::shared_ptr<PostgresDatabase> db,
                      bool store_enabled,
    ...
    virtual Result<void> save_all_results(const std::string& run_id,
                                         const Timestamp& date) = 0;

    // Common getters/setters
    void set_storage_enabled(bool enabled) { store_enabled_ = enabled; }
    bool is_storage_enabled() const { return store_enabled_; }
    std::string get_schema() const { return schema_; }
