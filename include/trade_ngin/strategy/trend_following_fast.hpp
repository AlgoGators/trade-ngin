...
/**
 * @brief Configuration specific to fast trend following strategy
 * Faster EMA windows and higher risk target for more frequent trading
 */
struct TrendFollowingFastConfig {
    double weight{1.0};                 // Weight for position sizing
    double risk_target{0.25};            // Target annualized risk level (higher than standard)
 ...
 */
struct TrendFollowingFastInstrumentData {
...
/**
 * @brief Multi-timeframe fast trend following strategy using EMA crossovers
 * Faster signals with shorter EMA windows for more frequent position changes
 */
class TrendFollowingFastStrategy : public BaseStrategy {
public:
    /**
     * @brief Constructor
     ...
     */
    TrendFollowingFastStrategy(std::string id, StrategyConfig config, TrendFollowingFastConfig trend_config,
     ...
     */
    const TrendFollowingFastInstrumentData* get_instrument_data(const std::string& symbol) const {
        auto it = instrument_data_.find(symbol);
        if (it != instrument_data_.end()) {
            return &it->second;
        }
     ...
     */
    const std::unordered_map<std::string, TrendFollowingFastInstrumentData>& get_all_instrument_data() const {
