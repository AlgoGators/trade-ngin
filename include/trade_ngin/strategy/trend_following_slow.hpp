...
/**
 * @brief Configuration specific to slow trend following strategy
 * Slower EMA windows and lower risk target for more conservative trading
 */
struct TrendFollowingSlowConfig {
    ...
    int vol_lookback_short{64};   // Short lookback for volatility calculation (longer)
 ...
 */
struct TrendFollowingSlowInstrumentData {
...
/**
 * @brief Multi-timeframe slow trend following strategy using EMA crossovers
 * Slower lookback periods and lower risk target for more conservative trading
 */
class TrendFollowingSlowStrategy : public BaseStrategy {
public:
    /**
     * @brief Constructor
     * @param id Strategy identifier
     * @param config Base strategy configuration
     * @param trend_config Slow trend following specific configuration
     * @param db Database interface
     * @param registry Instrument registry for accessing instrument data
     */
    TrendFollowingSlowStrategy(std::string id, StrategyConfig config, TrendFollowingSlowConfig trend_config,
     ...
     */
    const TrendFollowingSlowInstrumentData* get_instrument_data(const std::string& symbol) const {
        auto it = instrument_data_.find(symbol);
        if (it != instrument_data_.end()) {
            return &it->second;
        }
     ...
     */
    const std::unordered_map<std::string, TrendFollowingSlowInstrumentData>& get_all_instrument_data() const {
...
private:
    TrendFollowingSlowConfig trend_config_;
