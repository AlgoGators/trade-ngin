...
public:
     ...
     */
    template<typename T>
    T get_with_default(const std::string& section, const std::string& key, const T& default_value) const;
...
Result<T> CredentialStore::get(const std::string& section, const std::string& key) const {
    ...
    if (!config_.contains(section) || !config_[section].contains(key)) {
        return make_error<T>(
            ...
            "Configuration not found: " + section + "." + key,
            "CredentialStore"
        );
    }
...
template<typename T>
T CredentialStore::get_with_default(const std::string& section, const std::string& key, const T& default_value) const {
