...
public:
    MockStrategy(std::string id, StrategyConfig config, std::shared_ptr<DatabaseInterface> db)
    ...
    Result<void> on_data(const std::vector<Bar>& data) override {
        ...
        try {
            ...
            for (const auto& bar : data) {
                ...
                double price_change = bar.high.as_double() - bar.low.as_double();
                double volatility = price_change / bar.close.as_double();

                // Base position size - varies with price to test optimization
                double base_size = (config_.capital_allocation * 0.1) / bar.close.as_double();
                ...
                double signal = 2.0 * (bar.close.as_double() - 100.0) / 100.0;
            ...
            for (const auto& [symbol, pos] : positions_) {
                metrics_.total_pnl += pos.unrealized_pnl.as_double();
                ...
                if (it != data.end()) {
                    double daily_return =
                        (it->close.as_double() - it->open.as_double()) / it->open.as_double();
            ...
            return make_error<void>(
                ...
                std::string("Error processing data in mock strategy: ") + e.what(), "MockStrategy");
        }
    }
    ...
    Result<void> check_risk_limits() override {
        ...
        for (const auto& [symbol, pos] : positions_) {
            double position_value = std::abs((pos.quantity * pos.average_price).as_double());
        ...
        if (total_exposure > config_.capital_allocation * config_.max_leverage) {
            return make_error<void>(ErrorCode::RISK_LIMIT_EXCEEDED,
                                    "Total exposure exceeds maximum leverage", "MockStrategy");
        }

        double drawdown = -metrics_.total_pnl / config_.capital_allocation;
        if (drawdown > risk_limits_.max_drawdown.as_double()) {
            return make_error<void>(ErrorCode::RISK_LIMIT_EXCEEDED, "Drawdown exceeds limit",
                                    "MockStrategy");
        }
