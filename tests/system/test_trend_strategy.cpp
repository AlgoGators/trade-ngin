...
void TrendStrategy::runBacktest(const std::string& connection_string) {
    try {
        ...
        auto db_client = std::make_shared<DatabaseClient>(connection_string);
        auto data_handler = std::make_shared<TestOHLCVDataHandler>(db_client);
        ...
        for (const auto& symbol : all_symbols) {
            ...
            auto data_table = data_handler->getOHLCVArrowTable(start_date, end_date, {symbol});
...
void test_trend_strategy() {
    try {
        const std::string connection_string = "postgresql://localhost:5432/trade_ngin";
        
        // Create database connection
        auto db_client = std::make_shared<DatabaseClient>(connection_string);
        auto data_handler = std::make_shared<TestOHLCVDataHandler>(db_client);
        
        // Initialize strategy with reasonable default parameters
        TrendStrategy strategy(
            1000000.0,  // initial_capital: $1M
            0.20,       // vol_target: 20% annualized volatility target
            0.10,       // min_vol: 10% minimum volatility
            0.30,       // max_vol: 30% maximum volatility
            4.0         // max_leverage: 4x maximum leverage
        );
        ...
        // Run backtest
        strategy.runBacktest(connection_string);
    } catch (const std::exception& e) {
        std::cerr << "Error in test_trend_strategy: " << e.what() << std::endl;
    }
} 