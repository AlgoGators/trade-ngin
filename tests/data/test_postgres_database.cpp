...
TEST_F(PostgresDatabaseTest, TimezoneHandling) {
    ...
    auto table = result.value();
    if (table->num_rows() > 0) {
        auto timestamp_array = std::static_pointer_cast<arrow::TimestampArray>(
            table->column(0)->chunk(0));
        // Verify timestamps are in expected range
        EXPECT_GE(timestamp_array->Value(0), 
                 std::chrono::duration_cast<std::chrono::seconds>(
                     ny_time.time_since_epoch()).count());
        EXPECT_LE(timestamp_array->Value(table->num_rows() - 1),
                 std::chrono::duration_cast<std::chrono::seconds>(
