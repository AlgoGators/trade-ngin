...
// ──────────────────────────────────────────────────────────────────────────
// E2-F42 / M-08, the FUTURES runners' half: the parse and the broken-down form
// must be in the SAME frame.
//
// live_portfolio.cpp and live_portfolio_conservative.cpp parsed the CLI date with
// std::mktime (local midnight) and then read it back with std::localtime. That
// pair is self-consistent, which is why the defect never showed on a New York
// host -- but every OTHER consumer of the same instant already formats it through
// gmtime: the yesterday key handed to HolidayChecker::is_holiday, the position
// date written by format_utc_date, the order-id stamp
// (ExecutionManager::generate_date_string, made UTC by 04b79f61), the target
// handed to trading.get_trading_days. East of Greenwich the mktime/localtime pair
// and the gmtime consumers land on different days and the run splits in two.
//
// The fix moves BOTH halves to UTC in one commit, which is what the ledger row
// requires. These tests pin the pairing rather than either half alone: parsing
// UTC while still formatting local would be a NEW one-day split on a New York
// host, in the opposite direction, and a test of parse_utc_date on its own would
// not see it.
// ──────────────────────────────────────────────────────────────────────────
...
// Exactly what the runners do: parse the argument, then take the broken-down
// form the run's own date lines are printed from.
struct RunnerDateFrame {
    std::string run_day;        // now_tm, as the runner formats it for the email header
    std::string yesterday_key;  // gmtime(now - 24h), the key is_holiday is given
    std::string order_id_stamp; // generate_date_string(now), the execution key
};

RunnerDateFrame runner_frame(const std::string& arg) {
    std::chrono::system_clock::time_point now;
    EXPECT_TRUE(trade_ngin::core::parse_utc_date(arg, now));

    auto now_time_t = std::chrono::system_clock::to_time_t(now);
    std::tm now_tm_storage{};
    std::tm* now_tm = gmtime_r(&now_time_t, &now_tm_storage);
    EXPECT_NE(now_tm, nullptr);

    std::ostringstream day;
    day << (now_tm->tm_year + 1900) << "-" << std::setfill('0') << std::setw(2)
        << (now_tm->tm_mon + 1) << "-" << std::setfill('0') << std::setw(2) << now_tm->tm_mday;

    auto yesterday = now - std::chrono::hours(24);
    auto y_t = std::chrono::system_clock::to_time_t(yesterday);
    std::tm y_tm{};
    gmtime_r(&y_t, &y_tm);
    std::ostringstream ykey;
    ykey << std::put_time(&y_tm, "%Y-%m-%d");

    return {day.str(), ykey.str(),
            trade_ngin::ExecutionManager::generate_date_string(now)};
}

}  // namespace

// The three artefacts of one run must name the same day, on any host.
TEST(FuturesRunnerDateFrame, EveryArtefactOfOneRunNamesTheSameDay) {
    for (const char* zone : {"America/New_York", "UTC", "Asia/Tokyo"}) {
        ForceTimezone tz(zone);
        const auto f = runner_frame("2026-04-24");
        EXPECT_EQ(f.run_day, "2026-04-24") << "host zone " << zone;
        EXPECT_EQ(f.yesterday_key, "2026-04-23") << "host zone " << zone;
        EXPECT_EQ(f.order_id_stamp, "20260424") << "host zone " << zone;
    }
}

// New York, where this runs today, must be UNMOVED by the change: that is what
// makes the item byte-identical on this host and what the A/B is checked against.
TEST(FuturesRunnerDateFrame, NewYorkIsUnmovedByTheFix) {
    ForceTimezone tz("America/New_York");

    // The OLD pair: mktime to parse, localtime to break down.
    std::tm tm = {};
    std::istringstream ss("2026-04-24");
    ss >> std::get_time(&tm, "%Y-%m-%d");
    ASSERT_FALSE(ss.fail());
    const auto old_now = std::chrono::system_clock::from_time_t(std::mktime(&tm));
    auto old_t = std::chrono::system_clock::to_time_t(old_now);
    std::tm old_tm{};
    ASSERT_NE(localtime_r(&old_t, &old_tm), nullptr);
    std::ostringstream old_day;
    old_day << (old_tm.tm_year + 1900) << "-" << std::setfill('0') << std::setw(2)
            << (old_tm.tm_mon + 1) << "-" << std::setfill('0') << std::setw(2) << old_tm.tm_mday;

    const auto f = runner_frame("2026-04-24");
    EXPECT_EQ(old_day.str(), f.run_day)
        << "the two frames disagree on a New York host, so the change is not "
           "byte-identical here and the A/B must be re-read as class B";

    // The instants still differ by the UTC offset. That is the part the A/B
    // measures: `now` moves from 04:00Z to 00:00Z, which moves the 730-day
    // BETWEEN window one bar at its start edge.
    EXPECT_NE(old_now, std::chrono::system_clock::from_time_t(
                           std::chrono::system_clock::to_time_t(old_now) - 4 * 3600))
        << "sanity: the offset is not zero on this host";
}

// The frame the old pair actually produced east of Greenwich: a whole run one
// day early, with the gmtime consumers disagreeing with the localtime ones.
TEST(FuturesRunnerDateFrame, TheOldPairSplitTheRunInTwoOnAPositiveOffsetHost) {
    ForceTimezone tz("Asia/Tokyo");
    ...
    // What the localtime half believed.
    auto old_t = std::chrono::system_clock::to_time_t(old_now);
    std::tm old_tm{};
    ASSERT_NE(localtime_r(&old_t, &old_tm), nullptr);
    EXPECT_EQ(old_tm.tm_mday, 24) << "the local half read the date the operator typed";

    // What every gmtime consumer of the same instant got.
    EXPECT_EQ(trade_ngin::core::format_utc_date(old_now), "2026-04-23")
        << "the stored date, the T-1 lookup and the order id all landed a day early";
    EXPECT_EQ(trade_ngin::ExecutionManager::generate_date_string(old_now), "20260423");

    // After the fix both halves say the 24th.
    const auto f = runner_frame("2026-04-24");
    EXPECT_EQ(f.run_day, "2026-04-24");
    EXPECT_EQ(f.order_id_stamp, "20260424");
}
