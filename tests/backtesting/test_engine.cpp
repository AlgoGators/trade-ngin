...
public:
    ...
    Result<void> on_data(const std::vector<Bar>& data) override {
        ...
        for (const auto& bar : data) {
            ...
            if (it != last_prices_.end()) {
                ...
                double price_change = bar.close.as_double() - prev_price;

                // Generate orders based on price movement
                if (price_change > 0 && bar.close.as_double() > prev_price * 1.005) {
                    ...
                    pos.average_price = (pos.average_price * (pos.quantity - trade_size_) +
                                         bar.close.as_double() * trade_size_) /
                                        pos.quantity;
                    pos.last_update = bar.timestamp;
                    pos.unrealized_pnl = (bar.close.as_double() - pos.average_price.as_double()) *
                                         pos.quantity.as_double();

                    // Update position
                    update_position(bar.symbol, pos);
                } else if (price_change < 0 && bar.close.as_double() < prev_price * 0.995) {
                    // Sell signal on 0.5% decrease
                    Position pos = positions_[bar.symbol];
                    if (pos.quantity.as_double() >
                        trade_size_) {  // Ensure we maintain some position
                        double sold_quantity = std::min(
                            pos.quantity.as_double() - initial_position_size_, trade_size_);
                        pos.quantity -= sold_quantity;
                        pos.realized_pnl +=
                            (bar.close.as_double() - pos.average_price.as_double()) * sold_quantity;
                        pos.last_update = bar.timestamp;
                        pos.unrealized_pnl =
                            (bar.close.as_double() - pos.average_price.as_double()) *
                            pos.quantity.as_double();
            ...
            // Update last price
            last_prices_[bar.symbol] = bar.close.as_double();
        }
        ...
        if (positions_.empty()) {
            for (const auto& bar : data) {
                ...
                pos.quantity = initial_position_size_;
                pos.average_price = bar.close.as_double();
                pos.last_update = bar.timestamp;
                pos.unrealized_pnl = (bar.close.as_double() - pos.average_price.as_double()) *
                                     pos.quantity.as_double();
                pos.realized_pnl = 0.0;
    ...
    void set_fail_on_data(bool fail) {
        fail_on_data_ = fail;
    }
    void set_trade_size(double size) {
        trade_size_ = size;
    }
    void set_initial_position_size(double size) {
        initial_position_size_ = size;
    }
    int get_bars_received() const {
        return bars_received_;
    }
    int get_executions_received() const {
        return executions_received_;
    }
    int get_signals_received() const {
        return signals_received_;
    }
...
protected:
    void SetUp() override {
        ...
        config_.portfolio_config.opt_config.capital =
            config_.portfolio_config.initial_capital.as_double();
    ...
    std::vector<Bar> create_test_data(const std::vector<std::string>& symbols, int days,
                                      double starting_price = 100.0, double volatility = 0.02) {
        std::vector<Bar> bars;
    ...
    void patch_mock_db_to_return_test_data() {
        ...
        public:
            ...
            Result<std::shared_ptr<arrow::Table>> get_market_data(
                ...
                const Timestamp& end_date, AssetClass asset_class, DataFrequency freq,
                ...
                for (const auto& bar : test_data_) {
                    ...
                    status = open_builder.Append(bar.open.as_double());
                    if (!status.ok()) {
                        return make_error<std::shared_ptr<arrow::Table>>(
                            ErrorCode::DATABASE_ERROR,
                    ...
                    status = high_builder.Append(bar.high.as_double());
                    if (!status.ok()) {
                        return make_error<std::shared_ptr<arrow::Table>>(
                            ErrorCode::DATABASE_ERROR,
                    ...
                    status = low_builder.Append(bar.low.as_double());
                    if (!status.ok()) {
                        return make_error<std::shared_ptr<arrow::Table>>(
                            ErrorCode::DATABASE_ERROR,
                    ...
                    status = close_builder.Append(bar.close.as_double());
                    if (!status.ok()) {
                        return make_error<std::shared_ptr<arrow::Table>>(
                            ErrorCode::DATABASE_ERROR,
                ...
                // Create table
                auto table =
                    arrow::Table::Make(schema, {timestamp_array, symbol_array, open_array,
    ...
    try {
        ...
        if (component_result.is_ok()) {
            ...
            EXPECT_EQ(state, ComponentState::INITIALIZED)
                << "Backtest engine not in INITIALIZED state";
        }
...
TEST_F(BacktestEngineTest, BacktestWithRealMarketData) {
    ...
    // Check equity curve
    const auto& backtest_results = result.value();
    EXPECT_FALSE(backtest_results.equity_curve.empty()) << "Equity curve is empty";

    // Since this is a mock test with injected data, positions may or may not be
    // generated depending on price trends. We just verify the backtest completes
    // and returns results.
}
...
TEST_F(BacktestEngineTest, StrategyFailure) {
    ...
    // Since we're in a test environment, the StateManager registration might not
    // work consistently So we'll focus on the primary test case: strategy failure
    // should cause the backtest to fail
}
...
TEST_F(BacktestEngineTest, SaveAndLoadResults) {
    ...
    std::string test_run_id =
        "TEST_RUN_" + std::to_string(std::chrono::system_clock::now().time_since_epoch().count());
...
TEST_F(BacktestEngineTest, ErrorHandling) {
    ...
    config_.strategy_config.end_date =
        config_.strategy_config.start_date - std::chrono::hours(24);  // End before start
