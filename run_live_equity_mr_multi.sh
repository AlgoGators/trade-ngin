...
# Script to run live_equity_mean_reversion for multiple dates
# Usage:
#   ./run_live_equity_mr_multi.sh --from 2025-01-01 --to 2025-01-03
#   ./run_live_equity_mr_multi.sh --dates "2025-01-01 2025-01-02 2025-01-03"
#   ./run_live_equity_mr_multi.sh --last-n-days 5
#   ./run_live_equity_mr_multi.sh --from 2025-01-01 --to 2025-01-03 --send-email

set -e

SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
LIVE_EQUITY_BIN="$SCRIPT_DIR/build/bin/Release/live_equity_mean_reversion"

# Check if live_equity_mean_reversion binary exists
if [ ! -f "$LIVE_EQUITY_BIN" ]; then
    echo "Error: live_equity_mean_reversion binary not found at $LIVE_EQUITY_BIN"
...
usage() {
    ...
    echo "  --last-n-days N     Run for the last N trading days"
    echo "  --send-email        Pass --send-email flag to live_equity_mean_reversion"
    echo "  --parallel          Run dates in parallel (default: sequential)"
    echo "  --max-jobs N        Maximum parallel jobs (default: 4)"
    echo ""
    echo "Examples:"
    echo "  $0 --from 2025-01-01 --to 2025-01-05"
    echo "  $0 --dates \"2025-01-02 2025-01-03 2025-01-06\""
    echo "  $0 --last-n-days 5"
    echo "  $0 --from 2025-01-01 --to 2025-01-05 --send-email --parallel"
    exit 1
}
...
# Function to run live_equity_mean_reversion for a single date
run_single_date() {
    local date=$1
    local email_flag=$2
    echo "=========================================="
    ...
    if $PARALLEL; then
        ...
        local log_file="logs/live_equity_mr_${date}.log"
        mkdir -p logs
        "$LIVE_EQUITY_BIN" "$date" $email_flag 2>&1 | tee "$log_file"
        echo "Completed: $date (log: $log_file)"
    else
        "$LIVE_EQUITY_BIN" "$date" $email_flag
        echo ""
        echo "Completed: $date"
        echo ""
    fi
...
if [ -n "$LAST_N_DAYS" ]; then
    # Generate dates for last N days (skip weekends for equities)
    days_added=0
    days_back=0
    while [ $days_added -lt $LAST_N_DAYS ]; do
        if [[ "$OSTYPE" == "darwin"* ]]; then
            # macOS
            date_str=$(date -v -${days_back}d "+%Y-%m-%d")
            day_of_week=$(date -v -${days_back}d "+%u")
        else
            # Linux
            date_str=$(date -d "$days_back days ago" "+%Y-%m-%d")
            day_of_week=$(date -d "$days_back days ago" "+%u")
        fi
    ...
    # Generate dates from range (skip weekends)
...
# Run live_equity_mean_reversion for each date
if $PARALLEL; then
    # Export function and variables for parallel execution
    export -f run_single_date
    export LIVE_EQUITY_BIN
    export PARALLEL
