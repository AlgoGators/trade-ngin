...
int main(int argc, char* argv[]) {
    try {
        ...
        // Pre-run margin metadata validation for futures instruments only
        ...
        mr_config.save_positions = false;  // Disable automatic position saving (we'll do it manually)
        mr_config.save_signals = false;
        mr_config.save_executions = false;  // No executions in daily mode
        ...
        coordinator_config.asset_class = AssetClass::EQUITIES;
        ...
        auto previous_positions_result = db->load_positions_by_date("LIVE_EQUITY_MEAN_REVERSION", previous_date, "trading.positions");
        ...
        if (!two_days_ago_close_prices.empty() && !previous_positions.empty() && pnl_manager) {
            ...
            if (!yesterday_finalized_positions.empty()) {
                ...
                auto update_result = db->store_positions(yesterday_finalized_positions, "LIVE_EQUITY_MEAN_REVERSION", "trading.positions");
        ...
        if (!two_days_ago_close_prices.empty() && yesterday_total_pnl != 0.0 && !is_first_trading_day) {
            ...
            try {
                ...
                auto live_results = data_loader->load_live_results("LIVE_EQUITY_MEAN_REVERSION", previous_date);
            ...
            try {
                ...
                if (db_ptr) {
                    auto prev_agg = db_ptr->get_previous_live_aggregates("LIVE_EQUITY_MEAN_REVERSION", previous_date, "trading.live_results");
                    if (prev_agg.is_ok()) {
            ...
            try {
                auto margin_metrics = data_loader->load_margin_metrics("LIVE_EQUITY_MEAN_REVERSION", previous_date);
        ...
        try {
            auto db_ptr = std::dynamic_pointer_cast<PostgresDatabase>(db);
            if (db_ptr) {
                auto prev_agg = db_ptr->get_previous_live_aggregates("LIVE_EQUITY_MEAN_REVERSION", now, "trading.live_results");
                if (prev_agg.is_ok()) {
