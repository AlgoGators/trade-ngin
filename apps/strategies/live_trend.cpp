...
int main() {
    try {
        ...
        tf_config.save_positions =
            false;  // Disable automatic position saving (we'll do it manually)
        ...
        // Pre-warm strategy state so portfolio can pull price history for
        // optimization/risk
        ...
        // Process data through portfolio pipeline (optimization + risk), mirroring
        // backtest
        ...
        // Get optimized portfolio positions (integer-rounded after
        // optimization/risk)
        ...
        auto previous_positions_result =
            db->load_positions_by_date("LIVE_TREND_FOLLOWING", previous_date, "trading.positions");
        ...
        // Get current market prices for all symbols (both current and previous
        // positions)
        ...
        INFO("Requesting current prices for " + std::to_string(symbols_to_price.size()) +
             " symbols");
        for (const auto& symbol : symbols_to_price) {
            DEBUG("Requesting price for symbol: " + symbol);
        }

        auto current_prices_result =
            db->get_latest_prices(symbols_to_price, trade_ngin::AssetClass::FUTURES);
        ...
        if (current_prices_result.is_ok()) {
            ...
            INFO("Retrieved current prices for " + std::to_string(current_prices.size()) +
                 " symbols");
            ...
            ERROR("Failed to get current prices: " +
                  std::string(current_prices_result.error()->what()));
            ERROR(
                "This means unrealized PnL will be calculated using average prices "
                "as fallback");
        }
        ...
        for (auto& [symbol, current_position] : positions) {
            ...
            if (current_prices.find(symbol) != current_prices.end()) {
                ...
                WARN("No current market price available for " + symbol +
            ...
            if (prev_qty != 0.0 && current_qty != 0.0) {
                ...
                if (std::abs(qty_change) > 1e-6) {
                    if (qty_change < 0) {
                        ...
                        // For futures: realized_pnl = closed_quantity * (current_price -
                        // entry_price)
                        position_realized_pnl =
                            -qty_change * (current_market_price - prev_avg_price);
                    } else {
                        // Position increased - no realized PnL, just new average price
                        // The new average price should already be calculated by the
                        // strategy
            ...
            if (current_qty != 0.0) {
                // For futures: unrealized_pnl = quantity * (current_price -
                // average_price)
        ...
        // Also process any previous positions that are no longer held (for realized
        // PnL)
        ...
        for (const auto& [symbol, position] : positions) {
            if (position.quantity.as_double() != 0.0) {
                ...
                std::cout << std::setw(10) << symbol << " | " << std::setw(10) << std::fixed
                          << std::setprecision(2) << position.quantity.as_double() << " | "
                          << std::setw(10) << std::fixed << std::setprecision(2)
                          << position.average_price.as_double() << " | " << std::setw(12)
        ...
        std::cout << "Total Notional: $" << std::fixed << std::setprecision(2) << total_notional
                  << std::endl;
        ...
        for (const auto& [symbol, position] : positions) {
            if (position.quantity.as_double() != 0.0) {  // Only save non-zero positions
                ...
                validated_position.last_update =
                    std::chrono::system_clock::now();  // Use current time
                validated_position.unrealized_pnl = position.unrealized_pnl;
                validated_position.realized_pnl = position.realized_pnl;

                // Validate and convert average_price to ensure it's within Decimal
                // limits
                double avg_price_double = static_cast<double>(position.average_price);
                DEBUG("Validating position " + symbol +
                      " with average_price: " + std::to_string(avg_price_double));
                ...
                if (avg_price_double > DECIMAL_MAX || avg_price_double < -DECIMAL_MAX) {
                    WARN("Position " + symbol + " has average_price " +
                         std::to_string(avg_price_double) + " which exceeds Decimal limit (" +
                         std::to_string(DECIMAL_MAX) + "), using 1.0 instead");
                ...
                DEBUG("Position to save: " + symbol +
                      " qty=" + std::to_string(position.quantity.as_double()) + " price=" +
                      std::to_string(static_cast<double>(validated_position.average_price)));
            }
        }

        if (!positions_to_save.empty()) {
            INFO("Attempting to save " + std::to_string(positions_to_save.size()) +
                 " positions to database");
            DEBUG("Database connection status: " +
                  std::string(db->is_connected() ? "connected" : "disconnected"));

            auto save_result =
                db->store_positions(positions_to_save, "LIVE_TREND_FOLLOWING", "trading.positions");
            if (save_result.is_error()) {
                ERROR("Failed to save positions to database: " +
                      std::string(save_result.error()->what()));
                ERROR("Error code: " +
                      std::to_string(static_cast<int>(save_result.error()->code())));
            } else {
                INFO("Successfully saved " + std::to_string(positions_to_save.size()) +
                     " positions to database");
            }
        } else {
        ...
        // Compute portfolio-level snapshot metrics using RiskManager on today's
        // state
        ...
        if (!previous_positions.empty()) {
            ...
            try {
                ...
                std::string prev_query =
                    "SELECT current_portfolio_value FROM "
                    "trading.live_results WHERE strategy_id = "
                    "'LIVE_TREND_FOLLOWING' AND date = '" +
                    prev_date_ss.str() + "'";
                auto prev_result = db->execute_query(prev_query);
                if (prev_result.is_ok() && prev_result.value()->num_rows() > 0) {
                    // Extract portfolio value from result (simplified - would need proper
                    // Arrow table parsing) For now, use initial capital as fallback
            ...
            if (previous_portfolio_value > 0) {
                daily_return = (current_portfolio_value - previous_portfolio_value) /
        ...
        try {
            ...
            double total_return =
                0.0;  // For daily runs, this would be calculated from previous day
            ...
            std::string delete_query =
                "DELETE FROM trading.live_results WHERE "
                "strategy_id = 'LIVE_TREND_FOLLOWING' AND "
                "date = '" +
                date_ss.str() + "'";
            auto delete_result = db->execute_direct_query(delete_query);
            if (delete_result.is_error()) {
                WARN("Failed to delete existing live results: " +
                     std::string(delete_result.error()->what()));
            }

            // Then insert new results
            std::string query =
                "INSERT INTO trading.live_results "
                "(strategy_id, date, total_return, volatility, total_pnl, "
                "unrealized_pnl, "
                "realized_pnl, current_portfolio_value, portfolio_var, "
                "gross_leverage, "
                "net_leverage, portfolio_leverage, max_correlation, jump_risk, "
                "risk_scale, "
                "total_notional, active_positions, config) "
                "VALUES ('LIVE_TREND_FOLLOWING', '" +
                date_ss.str() + "', " + std::to_string(total_return) + ", " +
                std::to_string(volatility) + ", " + std::to_string(total_pnl) + ", " +
                std::to_string(total_unrealized_pnl) + ", " + std::to_string(total_realized_pnl) +
                ", " + std::to_string(current_portfolio_value) + ", " +
            ...
            if (results_save_result.is_error()) {
                ERROR("Failed to save trading results: " +
                      std::string(results_save_result.error()->what()));
            } else {
        ...
        std::string filename = "daily_positions_" + std::to_string(now_tm->tm_year + 1900) +
        ...
        if (position_file.is_open()) {
            position_file << "symbol,quantity,avg_price,market_price,notional,unrealized_pnl,"
                             "realized_pnl,forecast\n";
            for (const auto& [symbol, position] : positions) {
                double notional =
                    position.quantity.as_double() * position.average_price.as_double();
                double forecast = tf_strategy->get_forecast(symbol);
                ...
                position_file << symbol << "," << position.quantity.as_double() << ","
                              << position.average_price.as_double() << "," << market_price << ","
                              << notional << "," << position.unrealized_pnl.as_double() << ","
                              << position.realized_pnl.as_double() << "," << forecast << "\n";
            }
            position_file.close();
        ...
        try {
            ...
            if (email_init_result.is_error()) {
                ...
                strategy_metrics["Net Leverage"] =
                    total_notional / current_portfolio_value;  // Same as gross for this strategy
                strategy_metrics["Active Positions"] = active_positions;
                strategy_metrics["Total Notional"] = total_notional;

                // Note: Risk metrics (Volatility, Jump Risk, Risk Scale) are shown in
                // Risk Metrics section to avoid duplication
