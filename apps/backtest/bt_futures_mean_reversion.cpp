...
int main() {
    try {
        ...
        logger_config.filename_prefix = "bt_futures_mr";
        ...
        // Configure backtest parameters for FUTURES
        ...
        // Start date: 6 months ago (faster backtest for testing)
        std::tm start_tm = *now_tm;
        start_tm.tm_mon -= 6;  // 6 months instead of 2 years
        ...
        // CRITICAL: Set asset class to FUTURES
        config.strategy_config.asset_class = AssetClass::FUTURES;
        config.strategy_config.data_freq = DataFrequency::DAILY;
        config.strategy_config.data_type = "ohlcv";
        config.strategy_config.commission_rate = 2.25;   // $2.25 per contract
        ...
        // Test with a few liquid futures contracts
        // Energy, Metals, and Currency futures that might mean revert
        ...
        std::cout << "Testing with futures symbols: ";
        ...
        config.portfolio_config.initial_capital = 500000.0;  // $500K (futures need more capital)
        ...
        std::cout << "Commission rate: $" << config.strategy_config.commission_rate << " per contract"
                  << std::endl;
        ...
        mr_strategy_config.asset_classes = {AssetClass::FUTURES};
        mr_strategy_config.frequencies = {DataFrequency::DAILY};
        mr_strategy_config.max_drawdown = 0.25;  // 25% max drawdown for futures
        mr_strategy_config.max_leverage = 3.0;   // Higher leverage for futures
        mr_strategy_config.save_positions = false;
        mr_strategy_config.save_signals = false;
        mr_strategy_config.save_executions = false;

        // Add position limits for futures (contracts)
        for (const auto& symbol : config.strategy_config.symbols) {
            mr_strategy_config.position_limits[symbol] = 10.0;  // Max 10 contracts per symbol
        ...
        mr_config.entry_threshold = 2.5;       // Enter at 2.5 standard deviations (wider for futures)
        mr_config.exit_threshold = 0.5;        // Exit at 0.5 standard deviations
        mr_config.risk_target = 0.20;          // 20% annualized risk (higher for futures)
        mr_config.position_size = 0.15;        // 15% of capital per position
        mr_config.vol_lookback = 20;           // 20-day volatility
        mr_config.use_stop_loss = true;
        mr_config.stop_loss_pct = 0.08;        // 8% stop loss (wider for futures)

        // Create and initialize the strategy
        INFO("Initializing MeanReversionStrategy for futures...");
        ...
        auto mr_strategy = std::make_shared<MeanReversionStrategy>(
            "FUTURES_MEAN_REVERSION", mr_strategy_config, mr_config, db, registry_ptr);
        ...
        std::cout << "\n======= Futures Mean Reversion Backtest Results =======" << std::endl;
        ...
        try {
            auto save_result = engine->save_results_to_db(backtest_results, "futures_mean_reversion");
        ...
        try {
            auto csv_result = engine->save_results_to_csv(backtest_results, "futures_mean_reversion");
        ...
        std::cout << "\nBacktest complete! Check logs/ directory for detailed logs." << std::endl;

        return 0;

    } catch (const std::exception& e) {
        std::cerr << "FATAL ERROR: " << e.what() << std::endl;
        ERROR("Fatal exception: " + std::string(e.what()));
        return 1;
    }
}
