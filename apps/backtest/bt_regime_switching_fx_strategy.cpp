...
int main() {
    try {
        ...
        // Start from 2 years ago to ensure we have enough data
        ...
        // Strategy symbols
        config.strategy_config.symbols = {"6C", "6E", "6J", "6B", "6A"};
        ...
        fx_config.save_signals = true;
        fx_config.save_executions = true;

        // Add position limits and costs
        for (const auto& symbol : config.strategy_config.symbols) {
            fx_config.position_limits[symbol] = 100.0;  // Conservative limit
